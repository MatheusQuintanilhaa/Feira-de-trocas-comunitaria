...
class PropostaController {
  ...
  async createProposta(req, res) {
    ...
    if (itemOfertadoId === itemDesejadoId) {
      return res
        .status(400)
        .json({ message: "Um item não pode ser trocado por si mesmo." });
    }
    ...
    try {
      ...
      if (!itemOfertado || !itemDesejado) {
        return res
          .status(404)
          .json({ message: "Item ofertado ou item desejado não encontrado." });
      }
      ...
      if (itemOfertado.usuarioId !== ofertanteId) {
        return res
          .status(403)
          .json({ message: "Você só pode ofertar itens que possui." });
      }
      ...
      const newProposta = await prisma.proposta.create({
        ...
        select: {
          // Seleciona apenas os campos que devem ser retornados na resposta
          id: true,
          status: true,
          createdAt: true,
          itemOfertado: {
      ...
      return res
        .status(500)
        .json({ message: "Erro interno do servidor ao criar proposta." });
    }
  }
  ...
  async getAllPropostas(req, res) {
    ...
    try {
      ...
      return res
        .status(500)
        .json({ message: "Erro interno do servidor ao buscar propostas." });
    }
  }
  ...
  async getPropostaById(req, res) {
    ...
    try {
      ...
      return res
        .status(500)
        .json({ message: "Erro interno do servidor ao buscar proposta." });
    }
  }
  ...
  async acceptProposta(req, res) {
    ...
    try {
      ...
      if (error.code === "P2025") {
        ...
        return res
          .status(404)
          .json({ message: "Proposta não encontrada ou já processada." });
      }
      return res
        .status(500)
        .json({ message: "Erro interno do servidor ao aceitar proposta." });
    }
  }
  ...
  async deleteProposta(req, res) {
    ...
    try {
      ...
      const proposta = await prisma.proposta.findUnique({
        ...
        select: {
          ofertanteId: true,
          itemDesejado: { select: { usuarioId: true } },
        },
      });
  ...
  async getMyProposalHistory(req, res) {
    ...
    try {
      const propostas = await prisma.proposta.findMany({
        where: {
          OR: [{ ofertanteId: userId }, { donoItemDesejadoId: userId }],
        },
        select: {
          id: true,
          status: true,
